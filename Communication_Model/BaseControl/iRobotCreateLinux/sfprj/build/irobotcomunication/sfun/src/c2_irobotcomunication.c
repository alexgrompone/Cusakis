...
#include "c2_irobotcomunication.h"
...
static SFc2_irobotcomunicationInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c2_irobotcomunication(void);
static void initialize_params_c2_irobotcomunication(void);
static void enable_c2_irobotcomunication(void);
static void disable_c2_irobotcomunication(void);
static void finalize_c2_irobotcomunication(void);
static void sf_c2_irobotcomunication(void);
...
static void initialize_c2_irobotcomunication(void)
{
  ...
  chartInstance.c2_is_active_c2_irobotcomunication = 0U;
}
...
const mxArray *sf_c2_irobotcomunication_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c2_irobotcomunication_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_irobotcomunication_get_autoinheritance_info(void)
{
...
static void sf_opaque_initialize_c2_irobotcomunication(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c2_irobotcomunication();
  initialize_c2_irobotcomunication();
}

static void sf_opaque_enable_c2_irobotcomunication(void *chartInstanceVar)
{
  enable_c2_irobotcomunication();
}

static void sf_opaque_disable_c2_irobotcomunication(void *chartInstanceVar)
{
  disable_c2_irobotcomunication();
}

static void sf_opaque_gateway_c2_irobotcomunication(void *chartInstanceVar)
{
  sf_c2_irobotcomunication();
}

static void sf_opaque_terminate_c2_irobotcomunication(void *chartInstanceVar)
{
  if (sim_mode_is_rtw_gen(chartInstance.S) ||
   sim_mode_is_external(chartInstance.S)) {
    sf_clear_rtw_identifier(chartInstance.S);
  }
  finalize_c2_irobotcomunication();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_irobotcomunication(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c2_irobotcomunication(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("irobotcomunication",2);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("irobotcomunication",2,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"irobotcomunication",2,1);
      sf_mark_chart_reusable_outputs(S,"irobotcomunication",2,3);
    }
    sf_set_rtw_dwork_info(S,"irobotcomunication",2);
...
static void mdlRTW_c2_irobotcomunication(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "irobotcomunication", 2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_irobotcomunication(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c2_irobotcomunication;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c2_irobotcomunication;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c2_irobotcomunication;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c2_irobotcomunication;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c2_irobotcomunication;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c2_irobotcomunication;
  chartInstance.chartInfo.mdlStart = mdlStart_c2_irobotcomunication;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c2_irobotcomunication;
...
void c2_irobotcomunication_method_dispatcher(SimStruct *S, int_T method, void
 *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c2_irobotcomunication(S);
    break;
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_irobotcomunication(S);
    break;
   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_irobotcomunication(S);
    break;
   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
     "Error calling c2_irobotcomunication_method_dispatcher.\n"
