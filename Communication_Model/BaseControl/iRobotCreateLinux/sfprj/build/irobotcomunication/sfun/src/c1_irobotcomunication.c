...
static SFc1_irobotcomunicationInstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c1_irobotcomunication(void);
static void initialize_params_c1_irobotcomunication(void);
static void enable_c1_irobotcomunication(void);
static void disable_c1_irobotcomunication(void);
static void finalize_c1_irobotcomunication(void);
static void sf_c1_irobotcomunication(void);
...
static void initialize_c1_irobotcomunication(void)
{
  ...
  chartInstance.c1_is_active_c1_irobotcomunication = 0U;
}
...
const mxArray *sf_c1_irobotcomunication_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c1_irobotcomunication_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c1_irobotcomunication_get_autoinheritance_info(void)
{
...
static void sf_opaque_initialize_c1_irobotcomunication(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c1_irobotcomunication();
  initialize_c1_irobotcomunication();
}

static void sf_opaque_enable_c1_irobotcomunication(void *chartInstanceVar)
{
  enable_c1_irobotcomunication();
}

static void sf_opaque_disable_c1_irobotcomunication(void *chartInstanceVar)
{
  disable_c1_irobotcomunication();
}

static void sf_opaque_gateway_c1_irobotcomunication(void *chartInstanceVar)
{
  sf_c1_irobotcomunication();
}

static void sf_opaque_terminate_c1_irobotcomunication(void *chartInstanceVar)
{
  if (sim_mode_is_rtw_gen(chartInstance.S) ||
   sim_mode_is_external(chartInstance.S)) {
    sf_clear_rtw_identifier(chartInstance.S);
  }
  finalize_c1_irobotcomunication();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c1_irobotcomunication(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c1_irobotcomunication(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("irobotcomunication",1);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("irobotcomunication",1,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"irobotcomunication",1,2);
      sf_mark_chart_reusable_outputs(S,"irobotcomunication",1,5);
    }
    sf_set_rtw_dwork_info(S,"irobotcomunication",1);
...
static void mdlRTW_c1_irobotcomunication(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "irobotcomunication", 1);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c1_irobotcomunication(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c1_irobotcomunication;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c1_irobotcomunication;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c1_irobotcomunication;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c1_irobotcomunication;
  chartInstance.chartInfo.disableChart = sf_opaque_disable_c1_irobotcomunication;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c1_irobotcomunication;
  chartInstance.chartInfo.mdlStart = mdlStart_c1_irobotcomunication;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c1_irobotcomunication;
...
void c1_irobotcomunication_method_dispatcher(SimStruct *S, int_T method, void
 *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c1_irobotcomunication(S);
    break;
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c1_irobotcomunication(S);
    break;
   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c1_irobotcomunication(S);
    break;
   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
     "Error calling c1_irobotcomunication_method_dispatcher.\n"
