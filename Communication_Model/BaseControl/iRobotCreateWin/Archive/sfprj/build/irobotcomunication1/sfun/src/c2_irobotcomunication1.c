/* Include files */
#include "irobotcomunication1_sfun.h"
#include "c2_irobotcomunication1.h"
#define CHARTINSTANCE_CHARTNUMBER       (chartInstance.chartNumber)
#define CHARTINSTANCE_INSTANCENUMBER    (chartInstance.instanceNumber)
#include "irobotcomunication1_sfun_debug_macros.h"
...
static SFc2_irobotcomunication1InstanceStruct chartInstance;

/* Function Declarations */
static void initialize_c2_irobotcomunication1(void);
static void initialize_params_c2_irobotcomunication1(void);
static void enable_c2_irobotcomunication1(void);
static void disable_c2_irobotcomunication1(void);
static void finalize_c2_irobotcomunication1(void);
static void sf_c2_irobotcomunication1(void);
...
static void initialize_c2_irobotcomunication1(void)
{
  ...
  chartInstance.c2_is_active_c2_irobotcomunication1 = 0U;
}
...
static void sf_c2_irobotcomunication1(void)
{
  ...
  *c2_b_y3 = c2_y3;
  ...
  sf_debug_check_for_state_inconsistency(_irobotcomunication1MachineNumber_,
...
const mxArray *sf_c2_irobotcomunication1_get_eml_resolved_functions_info(void)
{
...
/* SFunction Glue Code */
void sf_c2_irobotcomunication1_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c2_irobotcomunication1_get_autoinheritance_info(void)
{
 ...
 fullDebuggerInitialization)
{
  if(ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
    ...
    if(!sim_mode_is_rtw_gen(S)) {
      {
        ...
        chartAlreadyPresent =
          sf_debug_initialize_chart(_irobotcomunication1MachineNumber_,
         2,
         1,
         1,
         4,
        ...
        if(chartAlreadyPresent==0) {
          ...
          init_script_number_translation(_irobotcomunication1MachineNumber_,chartInstance.chartNumber);
          sf_debug_set_chart_disable_implicit_casting(_irobotcomunication1MachineNumber_,chartInstance.chartNumber,1);
          sf_debug_set_chart_event_thresholds(_irobotcomunication1MachineNumber_,
           chartInstance.chartNumber,
           0,
           0,
           0);
    ...
    sf_debug_reset_current_state_configuration(_irobotcomunication1MachineNumber_,chartInstance.chartNumber,chartInstance.instanceNumber);
  }
}

static void sf_opaque_initialize_c2_irobotcomunication1(void *chartInstanceVar)
{
  chart_debug_initialization(chartInstance.S,0);
  initialize_params_c2_irobotcomunication1();
  initialize_c2_irobotcomunication1();
}

static void sf_opaque_enable_c2_irobotcomunication1(void *chartInstanceVar)
{
  enable_c2_irobotcomunication1();
}

static void sf_opaque_disable_c2_irobotcomunication1(void *chartInstanceVar)
{
  disable_c2_irobotcomunication1();
}

static void sf_opaque_gateway_c2_irobotcomunication1(void *chartInstanceVar)
{
  sf_c2_irobotcomunication1();
}

static void sf_opaque_terminate_c2_irobotcomunication1(void *chartInstanceVar)
{
  if (sim_mode_is_rtw_gen(chartInstance.S) ||
   sim_mode_is_external(chartInstance.S)) {
    sf_clear_rtw_identifier(chartInstance.S);
  }
  finalize_c2_irobotcomunication1();
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c2_irobotcomunication1(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c2_irobotcomunication1(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable("irobotcomunication1",2);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop("irobotcomunication1",2,"gatewayCannotBeInlinedMultipleTimes"));
    if(chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"irobotcomunication1",2,1);
      sf_mark_chart_reusable_outputs(S,"irobotcomunication1",2,3);
    }
    sf_set_rtw_dwork_info(S,"irobotcomunication1",2);
...
static void mdlRTW_c2_irobotcomunication1(SimStruct *S)
{
  if(sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "irobotcomunication1", 2);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c2_irobotcomunication1(SimStruct *S)
{
  chartInstance.chartInfo.chartInstance = NULL;
  chartInstance.chartInfo.isEMLChart = 1;
  chartInstance.chartInfo.chartInitialized = 0;
  chartInstance.chartInfo.sFunctionGateway =
    sf_opaque_gateway_c2_irobotcomunication1;
  chartInstance.chartInfo.initializeChart =
    sf_opaque_initialize_c2_irobotcomunication1;
  chartInstance.chartInfo.terminateChart =
    sf_opaque_terminate_c2_irobotcomunication1;
  chartInstance.chartInfo.enableChart = sf_opaque_enable_c2_irobotcomunication1;
  chartInstance.chartInfo.disableChart =
    sf_opaque_disable_c2_irobotcomunication1;
  chartInstance.chartInfo.zeroCrossings = NULL;
  chartInstance.chartInfo.outputs = NULL;
  chartInstance.chartInfo.derivatives = NULL;
  chartInstance.chartInfo.mdlRTW = mdlRTW_c2_irobotcomunication1;
  chartInstance.chartInfo.mdlStart = mdlStart_c2_irobotcomunication1;
  chartInstance.chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c2_irobotcomunication1;
...
void c2_irobotcomunication1_method_dispatcher(SimStruct *S, int_T method, void
 *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c2_irobotcomunication1(S);
    break;
   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c2_irobotcomunication1(S);
    break;
   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c2_irobotcomunication1(S);
    break;
   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
     "Error calling c2_irobotcomunication1_method_dispatcher.\n"
