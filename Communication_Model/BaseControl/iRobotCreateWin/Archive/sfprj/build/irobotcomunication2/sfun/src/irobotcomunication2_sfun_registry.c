...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_irobotcomunication2_process_check_sum_call( int nlhs,
   mxArray * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_irobotcomunication2_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return
  1;
  return 0;
}
...
{
  extern unsigned int sf_irobotcomunication2_autoinheritance_info( int nlhs,
   mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_irobotcomunication2_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
...
{
  extern unsigned int sf_irobotcomunication2_get_eml_resolved_functions_info(
  ...
  if(strcmp(machineName, "irobotcomunication2") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return
    sf_irobotcomunication2_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_irobotcomunication2_method_dispatcher(SimStruct *S,
...
{
  if(!strcmp(machineName,"irobotcomunication2")) {
    return(sf_irobotcomunication2_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }
  return 0;
}
extern void irobotcomunication2_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    irobotcomunication2_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void irobotcomunication2_initializer(void);
extern void irobotcomunication2_register_exported_symbols(SimStruct* S);
extern void irobotcomunication2_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  irobotcomunication2_register_exported_symbols(S);
}
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if(sfMachineGlobalInitializerCallable) {
    ...
    if(!simModeIsRTWGen) {
      irobotcomunication2_debug_initialize();
    }
    irobotcomunication2_initializer();
  }
  return;
}
